...
def train_model(model, loss_fcn, g, train_nid, device, metrics, args, val_nid, agg, stopper=None, reg_model=None):
    ...
    local_edges = g.local_partition.edata[dgl.EID].numpy()
   
    #----------------- Biased Neighbour Sampler-------------------#
    biased_start_time = time.time()
    total_edges = g.edata["__orig__"].shape[0]

    local_edges = g.local_partition.edata["_ID"]
    tmp_strings = ["w0","w1","w2","w3"]
    weight = tmp_strings[pb.partid]

    bns = BiasedNeighbourSampler(total_edges,local_edges,alpha=1000,prob=0)

    prob_arr,probab = bns.total_probability()
    print("before",probab[1256748])
    g.edata[weight] = probab
    print("af",g.edata[weight][1256748])
    ...
    print("Total Time taken for Biased is------>",biased_end_time-biased_start_time)
    ...
    for epoch in range(num_epochs):
        ...
        for step, (input_nodes, seeds, blocks) in enumerate(dataloader):
            ...
            for b,block in enumerate(blocks):
                ...
                tmp_allid = len(edge_ids)
                tmp_haloid = len(np.setdiff1d(edge_ids, local_edges))
                print("--> Part",pb.partid,"--> Block",b,"--> the ratio",tmp_haloid/tmp_allid,"--> Halo count",tmp_haloid,"--> All id",tmp_allid)
                print("  ")
                # print()
                block_p += tmp_haloid/tmp_allid
            #g.edata[weight][1000]
            # block_p = block_p
            probab_step.append(block_p)
                

            # ## OptimisedSampling
            # tmp_halo_eid = []
            # for b,block in enumerate(blocks):
            #     halo_eids = list(np.setdiff1d(np.array(block.edata[dgl.EID]), local_edges))
            #     tmp_halo_eid+=halo_eids
        ...
        if args['early_stop']:
            if stopper.step(val_scores[args["stopping_criteria"]], 1-int(agg), model, val_scores[-2+args["stopping_criteria"]]):
                ...
                break
    print("Average Prob for all epochs is---->",sum(prob_halo)/len(prob_halo))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--c_and_s", type=int, default=0, help="whether to apply correct and smooth")
    parser.add_argument("--ensemble", type=int, default=0, help="whether to apply ensemble")
